...
ConVar bot_attack("afkbot_flipout", "0", 0, "afkbots all attack");
ConVar bot_stop("afkbot_stop", "0", 0, "Make bots stop thinking!");
...
ConVar bot_shoot_breakables("afkbot_shoot_breakables", "1", 0, "if 1, bots will shoot breakable objects");
...
ConVar bot_notarget("afkbot_notarget", "0", 0, "bots don't shoot the host!");
...
ConVar bot_melee_only("afkbot_melee_only", "0", 0, "if 1 bots will only use melee weapons");
ConVar bot_debug_iglev("afkbot_debug_iglev", "0", 0, "bot think ignores functions to test cpu speed");
ConVar bot_dont_move("afkbot_dontmove", "0", 0, "if 1 , bots will all move forward");
ConVar bot_runplayercmd_hookonce("afkbot_runplayer_hookonce", "1", 0, "function will hook only once, if 0 it will unook and rehook after every map");
ConVar bot_ladder_offs("afkbot_ladder_offs", "42", 0, "difference in height for bot to think it has touched the ladder waypoint");
ConVar bot_ffa("afkbot_ffa", "0", 0, "Free for all mode -- bots shoot everyone");
ConVar bot_menu_update_time1("afkbot_menu_update_time1", "0.04", 0, "time to update menus [displaying message]");
ConVar bot_menu_update_time2("afkbot_menu_update_time2", "0.2", 0, "time to update menus [interval]");
ConVar bot_autowaypoint_dist("afkbot_autowpt_dist", "250.0", 0, "distance for autowaypoints to be placed");
ConVar bot_tooltips("afkbot_tooltips", "1", 0, "Enables/disables help tooltips");
ConVar bot_debug_notasks("afkbot_debug_notasks", "0", 0, "Debug command, stops bots from doing tasks by themselves");
ConVar bot_debug_dont_shoot("afkbot_debug_dont_shoot", "0", 0, "Debug command, stops bots from shooting everyone");
ConVar bot_debug_show_route("afkbot_debug_show_route", "0", 0, "Debug command, shows waypoint route to host");
ConVar bot_supermode("afkbot_supermode", "0", 0, "If 1 will make every bot skill and reaction much higher");


// DOD vars
ConVar bot_nocapturing("afkbot_dontcapture", "0", 0, "bots don't capture flags in DOD:S");
ConVar bot_prone_enemy_only("afkbot_prone_enemy_only", "1", 0, "if 1 bots only prone in DOD:S when they have an enemy");
...
ConVar bot_bots_form_squads("afkbot_bots_form_squads", "1", 0, "if 1, bots will form their own squads via voice commands");
...
ConVar bot_change_class("afkbot_change_classes", "0", 0, "bots change classes at random intervals");
ConVar bot_use_vc_commands("afkbot_voice_cmds", "1", 0, "bots use voice commands e.g. medic/spy etc");
...
ConVar bot_demo_jump("afkbot_enable_pipejump", "1", 0, "Enable experimental pipe jumping at rocket jump waypoints");
...
ConVar bot_taunt("afkbot_taunt", "0", 0, "enable/disable bots taunting");
