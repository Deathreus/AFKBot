...
void CBotTF2MedicHeal::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	CBotTF2 *tfBot = static_cast<CBotTF2 *>(pBot);

	m_pHeal = tfBot->GetHealingEntity();

	if (m_pHeal && !m_bHealerJumped)
	{
		Vector vVelocity;

		if (!CClassInterface::GetVelocity(m_pHeal, &vVelocity) || (vVelocity.Length2D() > 1.0f))
		{
			IPlayerInfo *pPI = playerhelpers->GetGamePlayer(m_pHeal)->GetPlayerInfo();

			if (pPI)
			{
				if (pPI->GetLastUserCommand().buttons & IN_JUMP)
				{
					m_bHealerJumped = true;
					m_vJump = CBotGlobals::EntityOrigin(m_pHeal);
				}
			}
		}
	}
	...
	{
		tfBot->ClearHealingEntity();
		pBot->GetNavigator()->RollBackPosition();
		Fail();
	}
	else if (tfBot->GetHealFactor(m_pHeal) == 0.0f)
	{
		tfBot->ClearHealingEntity();
		pBot->GetNavigator()->RollBackPosition();
		Fail();
...
void CBotTF2ShootLastEnemyPosition::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fTime == 0)
		m_fTime = TIME_NOW + RandomFloat(2.0f, 4.5f);
...
{
	if (!m_fWaitTime)
		m_fWaitTime = TIME_NOW + RandomFloat(5.0f, 10.0f);

	if (!pBot->HasSomeConditions(CONDITION_NEED_HEALTH))
		Complete();
	else if (m_fWaitTime < TIME_NOW)
		Fail();
	else
	{
		// TO DO
...
void CBotTF2WaitFlagTask::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	{
		if (m_bFind)
			m_fWaitTime = TIME_NOW + 5.0f;
		else
			m_fWaitTime = TIME_NOW + 10.0f;
	}
...
void CBotTF2AttackPoint::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (CTeamFortress2Mod::IsPlayerInvuln(pBot->GetEdict()))
		Fail();
	...
	{
		Complete(); // done
		tfBot->UpdateAttackDefendPoints();
	}
	else if (m_iArea && !CTeamFortress2Mod::m_ObjectiveResource.IsCPValid(iCpIndex, iTeam, TF2_POINT_ATTACK))
	{
		...
		m_fAttackTime = TIME_NOW + RandomFloat(30.0, 60.0);
	else if (m_fAttackTime < TIME_NOW)
		Complete();
	else
	{
		if (m_fTime == 0)
		...
		else
		{
			...
			if (tfBot->GetClass() == TF_CLASS_SPY)
			{
				if (tfBot->IsDisguised())
					pBot->PrimaryAttack(); // remove dIsguIse to capture

				tfBot->WantToDisguise(false);
...
void CBotTF2PushPayloadBombTask::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fPushTime == 0)
	{
		m_fPushTime = TIME_NOW + RandomFloat(10.0, 30.0);
		m_vRandomOffset = Vector(RandomFloat(-50, 50), RandomFloat(-50, 50), 0);
	}
	else if (m_fPushTime < TIME_NOW)
	{
		Complete();
	}
...
void CBotTF2DefendPayloadBombTask::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	if (m_fDefendTime == 0)
	{
		m_fDefendTime = TIME_NOW + RandomFloat(10.0f, 30.0f);
		m_vRandomOffset = Vector(RandomFloat(-150.0f, 150.0f), RandomFloat(-150.0f, 150.0f), 0);
	}
	else if (m_fDefendTime < TIME_NOW)
	{
		Complete();
	}
	else if (m_pPayloadBomb.Get() == NULL)
...
void CBotTF2DefendPoint::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	{
		m_fDefendTime = TIME_NOW + RandomFloat(30.0, 60.0);
...
void CBotTF2UpgradeBuilding::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (!m_fTime)
		m_fTime = TIME_NOW + RandomFloat(9.0f, 11.0f);
...
void CBotTF2UpgradeWeapon::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	CBotTF2 *tfBot = static_cast<CBotTF2 *>(pBot);
	...
	else if (tfBot->GetClass() != m_iClass)
	{
		Fail();
		return;
	}
	else if (!tfBot->CanUpgradeWeapon())
	{
		Complete();
		return;
	}
	...
	{
		if(!CClassInterface::MvMIsPlayerAtUpgradeStation(pBot->GetEdict()))
		{	// Re-adjust position
			pSchedule->AddTask(new CMoveToTask(*pSchedule->PassedVector()));
			pSchedule->AddTask(new CBotTF2UpgradeWeapon(tfBot->GetClass()));
			Complete();
		}
		else
			tfBot->UpgradeWeapon();
...
void CBotTaskEngiPickupBuilding::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (tfBot->SentryRecentlyHadEnemy())
	{
		Fail();
		return;
	}
	...
	tfBot->UpdateCarrying();

	if (tfBot->IsCarrying()) //if ( CBotGlobals::EntityOrigin(m_pBuilding) == CBotGlobals::EntityOrigin(pBot->GetEdict()) )
		Complete();
	else if (m_fTime < TIME_NOW)
		Fail();
	else if (!pWeapon)
...
void CBotTaskEngiPlaceBuilding::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fTime == 0.0f)
	{
		...
		if (CTeamFortress2Mod::BuildingNearby((TFTeam)pBot->GetTeam(), m_vOrigin))
...
void CBotBackstab::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (!m_fTime)
		m_fTime = TIME_NOW + RandomFloat(5.0f, 10.0f);

	pBot->SetLookAtTask(LOOK_EDICT);
	pBot->LookAtEdict(m_pEnemy);
	...
	else if (!m_pEnemy || !CBotGlobals::EntityIsValid(m_pEnemy) || !CBotGlobals::EntityIsAlive(m_pEnemy))
	{
		if (pBot->GetEnemy() && (m_pEnemy != pBot->GetEnemy()) && pBot->HasSomeConditions(CONDITION_SEE_CUR_ENEMY) && CBotGlobals::IsAlivePlayer(pBot->GetEnemy()))
		{
			m_pEnemy = pBot->GetEnemy();
		}
		else
		{
			Fail();
	...
	else if (!pBot->IsVisible(m_pEnemy))
	{
		// thIs guy will do
		if (pBot->GetEnemy() && (m_pEnemy != pBot->GetEnemy()) && pBot->HasSomeConditions(CONDITION_SEE_CUR_ENEMY) && CBotGlobals::IsAlivePlayer(pBot->GetEnemy()))
		{
	...
	AngleVectors(CBotGlobals::EntityEyeAngles(m_pEnemy), &vangles);
	vrear = CBotGlobals::EntityOrigin(m_pEnemy) - (vangles * 45) + Vector(0, 0, 32);
	...
	{
		// uncloak
		if (CTeamFortress2Mod::IsPlayerCloaked(pBot->GetEdict()))
			pTF2Bot->SpyUnCloak();
		else
			pTF2Bot->HandleAttack(pBotWeapon, m_pEnemy);
	}
}
...
void CBotDefendTask::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	if (m_fTime == 0)
	{
		...
		{
			m_fTime = TIME_NOW + RandomFloat(20.0f, 90.0f);
	...
	float fDist = pBot->DistanceFrom(m_vOrigin);
...
void CBotTF2EngiLookAfter::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (!m_fTime)
	{
		m_fTime = TIME_NOW + RandomFloat(21.0f, 60.0f);
		m_fHitSentry = TIME_NOW + RandomFloat(1.0f, 3.0f);
	}
	else if (m_fTime < TIME_NOW)
	{
		tfBot->NextLookAfterSentryTime(TIME_NOW + RandomFloat(20.0f, 50.0f));
		Complete();
	}
	else
	{
		...
		if (pSentry != NULL)
		{
			...
			else
			{
				...
				if ((CClassInterface::GetSentryEnemy(pSentry) != NULL) ||
					(CClassInterface::GetObjectHealth(pSentry) < CClassInterface::GetObjectMaxHealth(pSentry)) ||
					(CClassInterface::GetSentryRockets(pSentry) < 20) ||
					(CClassInterface::GetSentryShells(pSentry) < 150))
				{
					pBot->PrimaryAttack();
				}
...
#if defined USE_NAVMESH
CBotTFEngiBuildTask::CBotTFEngiBuildTask(eObjectType iObject, Vector vSpot)
{
	m_iObject = iObject;
	m_vOrigin = vSpot;
	m_iState = 0;
	...
	INavMeshArea *area = g_pNavMesh->GetArea(vSpot);
	m_fRadius = (area->GetSWCornerZ() - area->GetNECornerZ()) / 2;
	m_iArea = area->GetID();
}
#else
CBotTFEngiBuildTask::CBotTFEngiBuildTask(eObjectType iObject, CWaypoint *pWaypoint)
{
	m_iObject = iObject;
...
void CBotTFEngiBuildTask::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fTime == 0.0f)
	{
		...
		m_fTime = TIME_NOW + RandomFloat(4.0f, 8.0f);

		m_fNextUpdateAngle = TIME_NOW + 0.5f;
	}
	else if (m_fTime < TIME_NOW)
		Fail();
	...
	QAngle qAim;
	qAim.Init();
	if(((CNavMeshNavigator*)pBot->GetNavigator())->ComputeAimVector(&qAim))
	{
		Vector vForward;
		AngleVectors(qAim, &vForward);
		m_vAimingVector = m_vOrigin + vForward * 1024.0f;
	}
#endif

	if (m_iObject == OBJ_DISP)
	{
	...
	if ((m_iTries > 1) && (m_fNextUpdateAngle < TIME_NOW))
	{
		QAngle angles;
		...
		m_fNextUpdateAngle = TIME_NOW + 1.5f;
	}
...
void CFindPathTask::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_bGetPassedIntAsWaypointId)
	{
		...
		if (!CWaypoints::ValidWaypointIndex(m_iWaypointId))
		{
			Fail();
			return;
		}
	...
	{
		pBot->m_ftWaypointStuckTime.Invalidate();
		...
		{
			pBot->m_ftWaypointStuckTime = RandomFloat(10.0f, 15.0f);
	...
	{
		if (!pBot->m_ftWaypointStuckTime.HasStarted())
			pBot->m_ftWaypointStuckTime = RandomFloat(5.0f, 10.0f);
...
void CBotTF2FindPipeWaypoint::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (CTeamFortress2Mod::IsPlayerInvuln(pBot->GetEdict()))
		Fail();
	else if (CTeamFortress2Mod::IsPlayerKrits(pBot->GetEdict()))
	...
	if (m_i == m_WaypointsI.size())
	{
		...
		else
		{
			...
			pSchedule->PassVector(const_cast<Vector *>(&CWaypoints::GetWaypoint(m_iNearesti)->GetOrigin()));
...
void CBotTF2TauntTask::Init()
{
	m_fTime = 0;
}

void CBotTF2TauntTask::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	if (m_fTime == 0)
		m_fTime = TIME_NOW + RandomFloat(2.5f, 5.0f);
	else if (m_fTime < TIME_NOW)
	{
		Fail();
		return;
...
CMoveToTask::CMoveToTask(Vector vOrigin)
{
	m_vVector = vOrigin;
	m_pEdict = MyEHandle();
...
void CMoveToTask::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if ((fDistance < 64) || (m_fPrevDist && (m_fPrevDist < fDistance)))
	{
		Complete();
		return;
	}
...
void CBotTFRocketJump::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	else
	{
		if (!m_fTime)
		{
			m_fTime = TIME_NOW + RandomFloat(4.0f, 5.0f);
		}

		if (((CBotTF2*)pBot)->RocketJump(&m_iState, &m_fJumpTime) == BOT_FUNC_COMPLETE)
			Complete();
		else if (m_fTime < TIME_NOW)
		{
			Fail();
...
void CSpyCheckAir::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fTime == 0.0f)
	{
		...
		m_fTime = TIME_NOW + RandomFloat(2.0f, 5.0f);
		m_fNextCheckUnseen = TIME_NOW + 0.1f;
	}
	...
	if ((m_pUnseenBefore == NULL) && (m_fNextCheckUnseen < TIME_NOW))
	{
		int i;
		edict_t *pPlayer;
		IGamePlayer *pPl;
	...
	else
	{
		if (RandomInt(0, 100) < iAttackProb)
		{
			...
			if (m_pUnseenBefore != NULL)
			{
				...
				{
					m_bHitPlayer = true;
					m_fTime = TIME_NOW + RandomFloat(0.5f, 1.5f);
				}
			}
		...
		{
			IPlayerInfo *pPI = playerhelpers->GetGamePlayer(m_pUnseenBefore)->GetPlayerInfo();

			// stop if I see the player im hitting attacking
			if (pPI->GetLastUserCommand().buttons & IN_ATTACK)
			{
				Complete();
				return;
			}
...
void CBotRemoveSapper::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fTime == 0.0f)
	{
		m_fTime = TIME_NOW + RandomFloat(8.0f, 12.0f);
	...
	if (m_id == OBJ_DISP)
	{
		...
		{
			if (m_fHealTime == 0.0f)
				m_fHealTime = TIME_NOW + RandomFloat(2.0f, 3.0f);
			else if (m_fHealTime < TIME_NOW)
			{
				Complete();
				return;
			}
...
void CBotTF2SnipeCrossBow::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fTime == 0.0f)
	{
		...
		m_fEnemyTime = 0.0f;
		m_fAimTime = 0.0f;
		m_fTime = TIME_NOW + RandomFloat(40.0f, 90.0f);
		//pBot->SecondaryAttack();
		pBot->ResetLookAroundTime();
	...
	else if ((m_iHideWaypoint != -1) && (((CBotFortress*)pBot)->IncomingRocket(512.0f) || (m_fHideTime > TIME_NOW)))
	{
	...
	else
	{
		...
		if (pBot->IsHoldingPrimaryAttack() && pBot->HasEnemy())
		{
			if (pBot->HasSomeConditions(CONDITION_SEE_CUR_ENEMY))
			{
				...
				m_fEnemyTime = TIME_NOW;

				if (m_fAimTime == 0.0f)
					m_fAimTime = TIME_NOW + RandomFloat(0.1f, 0.3f);
			...
			if (CTeamFortress2Mod::HasRoundStarted())
			{
				...
				if (angle > 0.96f) // 16 degrees
				{
					...
					if (m_iPrevClip > pBotWeapon->GetAmmo(pBot))
					{
						...
						if (bhide)
							m_fHideTime = TIME_NOW + RandomFloat(1.5f, 2.5f);
...
void CBotTF2Snipe::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	{
		if (CTeamFortress2Mod::IsPlayerZoomed(pBot->GetEdict()))
			pBot->SecondaryAttack();
	...
	else
	{
		...
		if (CTeamFortress2Mod::IsPlayerZoomed(pBot->GetEdict()) && pBot->HasEnemy())
		{
			if (pBot->HasSomeConditions(CONDITION_SEE_CUR_ENEMY))
			{
				m_vEnemy = CBotGlobals::EntityOrigin(pBot->GetEnemy());
...
void CBotTF2SpySap::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fTime == 0.0f)
	{
		m_fTime = TIME_NOW + RandomFloat(4.0f, 6.0f);
...
void CBotInvestigateHidePoint::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	if (m_fTime == 0)
	{
		m_fTime = TIME_NOW + RandomFloat(6.0f, 8.0f);
	...
	else if (m_fTime < TIME_NOW)
	{
		...
		{
			m_fTime = TIME_NOW + RandomFloat(1.0f, 2.0f);
			m_iState = 2;
		}
	}
	...
	if (m_CheckPoints.size() == 0)
	{
		...
		if (m_fInvestigateTime < TIME_NOW)
			Complete();
	}
	else
	{
		...
		case 0: // goto m_vMoveTo
			...
			{
				m_iState = 1;
				m_fInvestigateTime = TIME_NOW + RandomFloat(0.3f, 1.3f);
			}
			pBot->SetLookVector(m_vMoveTo);
			pBot->SetLookAtTask(LOOK_VECTOR);
			break;
...
{
	if (m_fTime == 0)
		m_fTime = TIME_NOW + RandomFloat(2.0, 4.0);
...
void CFollowTask::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (pBot->IsVisible(m_pFollow))
	{
		...
		m_fFollowTime = TIME_NOW + 1.0f;
	}

	if (m_fFollowTime < TIME_NOW)
		Complete();
}
...
void CCrouchHideTask::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fHideTime == 0)
		m_fHideTime = TIME_NOW + RandomFloat(7.0f, 14.0f);

	if (m_fChangeTime == 0.0f)
		m_fChangeTime = TIME_NOW + RandomFloat(0.9f, 2.9f);
	...
	if (m_fChangeTime < TIME_NOW)
	{
		m_bCrouching = !m_bCrouching;
		m_fChangeTime = TIME_NOW + RandomFloat(1.0f, 3.0f);
	}
...
void CHideTask::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fHideTime == 0)
		m_fHideTime = TIME_NOW + RandomFloat(5.0, 10.0);

	if (m_fHideTime < TIME_NOW)
		Complete();
}

CBotTF2DemomanPipeJump::CBotTF2DemomanPipeJump(CBot *pBot, Vector vWaypointGround, Vector vWaypointNext, CBotWeapon *pWeapon)
{
	m_iStartingAmmo = pWeapon->GetClip1(pBot);
	m_vStart = vWaypointGround - Vector(0, 0, 48.0f);
	m_vEnd = vWaypointNext;
	m_pPipeBomb = MyEHandle();
	m_fTime = 0;
	m_iState = 0;
	m_pWeapon = pWeapon;
	m_bFired = false;
...
void CBotTF2DemomanPipeJump::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	case 0:
		...
		else
		{
			...
			{
				// find pipe bomb
				m_pPipeBomb = servertools->FindEntityByClassnameNearest("tf_projectile_pipe_remote", pBot->GetOrigin(), 150.0f);

				if (m_pPipeBomb)
				{
					// Set thIs up incase of Fail, the bot knows he Has a sticky there
					tfBot->SetStickyTrapType(m_vStart, TF_TRAP_TYPE_ENEMY);
					m_iState++;
				}
				else
					Fail();
...
void CBotTF2DemomanPipeEnemy::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fTime == 0)
	{
		...
		tfBot->SetStickyTrapType(m_vEnemy, TF_TRAP_TYPE_ENEMY);
	...
	if (!CBotGlobals::EntityIsValid(m_pEnemy) || !CBotGlobals::EntityIsAlive(m_pEnemy) || (m_fTime < TIME_NOW))
	{
...
void CBotTF2DemomanPipeTrap::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (((CBotTF2*)pBot)->DeployStickies(m_iTrapType, m_vStand, m_vLocation, m_vSpread, &m_vPoint, &m_iState, &m_iStickies, &bFail, &m_fTime, m_iWptArea))
	{
		Complete();
	...
	case 0:
		{
			...
			if (!m_fTime)
				m_fTime = TIME_NOW + RandomFloat(3.5f, 8.0f);
		}
	...
	case 1:
		{
			...
			if (!m_fTime)
				m_fTime = TIME_NOW + RandomFloat(3.5f, 6.5f);
		}
		break; // say some random voice commands
	case 2:
		{
			if (!m_fTime)
			...
			if (!m_fTime)
				m_fTime = TIME_NOW + RandomFloat(1.5f, 3.0f);
		}
		break; // press some random buttons, such as attack2, jump
	case 3:
		{
			if (RandomInt(0, 1))
				pBot->Jump();
...
void CBotTF2Spam::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fTime == 0.0f)
	{
		...
		m_fNextAttack = TIME_NOW + RandomFloat(0.2f, 1.0f);
		m_fTime = TIME_NOW + RandomFloat(12.0f, 24.0f);
	}
	else if (m_fTime < TIME_NOW)
	{
		Complete();
	...
	else if (m_fNextAttack < TIME_NOW)
	{
		pBot->PrimaryAttack();
		m_fNextAttack = TIME_NOW + RandomFloat(0.2f, 1.0f);
	}
...
void CBotTF2AttackSentryGunTask::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if (m_fTime == 0.0f)
	{
		...
		m_fTime = TIME_NOW + RandomFloat(8.0f, 14.0f);
...
void CBotNest::Execute(CBot *pBot, CBotSchedule *pSchedule)
{
	...
	if ((tfBot->GetClass() == TF_CLASS_MEDIC) && tfBot->SomeoneCalledMedic())
		Fail(); // Follow player

	if (!tfBot->WantToNest())
	{
		Complete();
		tfBot->AddVoiceCommand(TF_VC_GOGOGO);
		return;
	}
	...
	if (m_fTime == 0)
	{
		m_fTime = TIME_NOW + RandomFloat(6.0f, 12.0f);

		if (RandomInt(0, 1))
			tfBot->AddVoiceCommand(TF_VC_HELP);
	}
	else if (m_fTime < TIME_NOW)
	{
		Complete();
...
{
m_fEnemyTime = 0.0f;
m_fTime = TIME_NOW + RandomFloat(20.0f,40.0f);
pBot->ResetLookAroundTime();
}
...
if ( m_fTimeout == 0.0f )
m_fTimeout = TIME_NOW;
else if ( (m_fTimeout + 3.0f) < TIME_NOW )
Fail();
}
else
m_fTimeout = 0.0f;

m_fScopeTime = TIME_NOW + RandomFloat(0.5f,1.0f);
}
}
...
if ( (m_fEnemyTime + m_fTime) < TIME_NOW )
{
if ( bDeployedOrZoomed )
pBot->SecondaryAttack();
...
m_fTime = TIME_NOW + RandomFloat(3.0f,5.0f);
...
{
m_fSecAttTime = 0;
m_fTime = TIME_NOW + RandomFloat(2.0f,4.0f);
}
...
{
if ( m_fSecAttTime < TIME_NOW )
{
pBot->SecondaryAttack();
m_fSecAttTime = TIME_NOW + RandomFloat(0.25,0.75);
}
}
}
}
...
CBotTF2EngineerInterrupt::CBotTF2EngineerInterrupt(CBot *pBot)
{
	...
	{
		m_fPrevSentryHealth = CClassInterface::GetObjectHealth(m_pSentryGun);
	}
	else
...
bool CBotTF2EngineerInterrupt::IsInterrupted(CBot *pBot, bool *bFailed, bool *bCompleted)
{
	if(m_pSentryGun.IsValid())
	{
		...
		{
			float m_fCurrentHealth = CClassInterface::GetObjectHealth(m_pSentryGun);
...
{
	return (this->m_fTimeOut != 0) && (TIME_NOW < this->m_fTimeOut);
}
...
const char *CBotTask::DebugString()
{
	try
	{
		std::type_info const &info = typeid(*this);
